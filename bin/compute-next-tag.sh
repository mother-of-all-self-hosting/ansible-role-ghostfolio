...
#
# Tags look like `v<Ghostfolio version>-<release>`:
#
# - if defaults/main.yml points at a Ghostfolio version that has never been
#   released, the release counter restarts at 0 (`v3.54.0-0`)
# - otherwise the counter is incremented (`v3.54.0-1`), but only if something
...
# Anchored on `ghostfolio_version:` so that neither the similarly named
# `ghostfolio_container_image_self_build_repo_version:` nor a commented-out
# line can be mistaken for it.
version="$(sed -nE 's|^ghostfolio_version:[[:space:]]*"?([^"[:space:]]+)"?.*$|\1|p' "$defaults_path" | head -n1)"

if [ -z "$version" ]; then
	echo >&2 "Could not determine the Ghostfolio version from $defaults_path"
	exit 1
fi

# The tags of this repository carry a leading `v` (`v3.54.0-0`) while the
# version does not (`3.54.0`), and a version which does carry one must not end
# up with two.
